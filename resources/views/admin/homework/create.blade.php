...
<head>
    ...
    <title>Create Homework | {{ config('app.name', 'School Portal') }}</title>
    ...
    <div class="container">
        <header>
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
                <div>
                    <h1><i class="fas fa-plus-circle"></i> Create Homework</h1>
                    <p class="subtitle">Add a new homework assignment for a specific class</p>
                </div>
        ...
        <div class="card">
            ...
            <form method="POST" action="{{ route('admin.homework.store') }}">
                @csrf
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="e.g., Chapter 5 Exercises" required>
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" placeholder="Provide detailed instructions for the homework assignment..." required>{{ old('description') }}</textarea>
                        @error('description')
                            <div class="error-text">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        @error('class_id')
                            <div class="error-text">{{ $message }}</div>
                        @enderror
                        <div class="help-text">Homework is created inside one class so it stays separated from other classes.</div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="submission_type" name="submission_type" required>
                            <option value="written" {{ old('submission_type') == 'written' ? 'selected' : '' }}>Written (Text)</option>
                            <option value="file" {{ old('submission_type') == 'file' ? 'selected' : '' }}>File Upload</option>
                        </select>
                        @error('submission_type')
                            <div class="error-text">{{ $message }}</div>
                        @enderror
                        <div class="help-text"><i class="fas fa-lightbulb"></i> Choose written for text answers or file for document submissions</div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @error('due_date')
                            <div class="error-text">{{ $message }}</div>
                        @enderror
                        <div class="help-text">Optional - leave blank for no deadline</div>
                    </div>
                </div>
