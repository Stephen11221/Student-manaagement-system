...
@section('content')
    <div class="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-4 lg:grid-cols-3">
            <div class="rounded-3xl border border-slate-200/10 bg-slate-950/70 p-5 lg:col-span-2">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-xl font-semibold text-white">Monthly trend</h2>
                        <p class="text-sm text-slate-400">Income versus expenses over the selected range.</p>
                    </div>
                </div>
                <div class="mt-4 h-80">
    ...
    <script>
        const accountingSeries = @json($monthlySeries);
        const ctx = document.getElementById('accountingTrendChart');

        if (ctx) {
            new Chart(ctx, {
                type: 'line',
                data: {
                    labels: accountingSeries.map((item) => item.month),
                    datasets: [
                        {
                            label: 'Income',
                            data: accountingSeries.map((item) => item.income),
                            borderColor: '#22d3ee',
                        ...
                        {
                            label: 'Expenses',
                            data: accountingSeries.map((item) => item.expenses),
                            borderColor: '#fb7185',
